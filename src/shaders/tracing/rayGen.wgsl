...
override ENABLE_RIS: bool = true;
override ENABLE_TEMPORAL: bool = true;
override WIDTH: u32;
override HEIGHT: u32;
...
fn main(@builtin(global_invocation_id) GlobalInvocationID: vec3u, @builtin(workgroup_id) WorkgroupID: vec3u) {
    ...
    // compute Weight
        ...
        if ENABLE_DI {
            ...
            pHat = bsdfLuminance * geometryTerm_luminance;
            if pHat > 0.0 {
                reservoirCurDI.W = reservoirCurDI.w_sum / max(1e-3, pHat) / f32(reservoirCurDI.M);
            } else {
                reservoirCurDI.W = 0.0;
                reservoirCurDI.w_sum = 0.0;
            }
        ...
        // GI
        if ENABLE_GI {
            reservoirCurGI.W = reservoirCurGI.w_sum / max(1e-3, luminance(reservoirCurGI.Lo)) / f32(reservoirCurGI.M);
        }
    }
